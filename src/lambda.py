...
def lambda_handler(event, context):
    ...
    # Writing sanitize data to DynamoDB
    df = writeFirstTable()
    writeSecondTable(df)
...
def writeSecondTable(df):
    ...
    sleep_latency["school_percent_deviation"] = round(100*(sleep_latency["school_latency"] - mean_school_latency)/(mean_school_latency), 1)
    sleep_latency["summer_percent_deviation"] = round(100*(sleep_latency["summer_latency"] - mean_summer_latency)/(mean_summer_latency), 1)
